...
function Message() {
    ...
    const postMsg = (e) => {   //Post Button functionality
    ...
    const taskList = task.map((obj, index) => {   // Your task list
        console.log(obj)
        return (
            <div>
                ...
                <h2>{index + 1} </h2>
                <h3>{obj.text}</h3>
                {obj.img ? <img src={obj.img} height="200px" width="150px" alt='abc'></img> : ''}
                {/* <button onClick={() => { deleteTask(index) }}>Delete</button> */}
            </div>
        )
    })

    // function deleteTask(index) {         // This is the delete button functionality
    ...
    return (
        <>
            <Grid className="App">
                    ...
                    <Grid container >
                        ...
                        <Grid item xs="10">
                            <TextareaAutosize
                                ...
                                style={{ width: "440px", height: "100px", border: "none" }}
                                onChange={(e) => { setNewTask(e.target.value) }}
                                value={newTask}

                            ></TextareaAutosize>
                            {newGif ?
                                <img className="selected-img" src={newGif} alt="abc"></img> : ""
                            }
                ...
                <Grid className='row' style={{ display: "flex", marginBottom: "15px" }}>
                ...
                <Grid className='footer'>
                    <Grid container style={{ marginLeft: "-15px", }}>
                        <Grid item xs={6} >
                            <Grid id='myDIV'>
                                <Input className='gifName' type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search Gif" />

                                <Button className='gif-srch-btn' onClick={searchGif}>Search</Button>
                            </Grid>

                            <Grid item>

                                <Dialog className='dialog-div' 
                                    open={open}
                                    onClose={handleClose}
                                >
